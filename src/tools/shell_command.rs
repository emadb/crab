...
impl Tool for ShellCommand {
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<String, ToolError> {
        let args: ShellCommandArgs = serde_json::from_value(args).map_err(|e| ToolError(e.to_string()))?;

        let child = tokio::process::Command::new("sh").arg("-c").arg(args.command)
            .current_dir(".")
        ...
        let result = match tokio::time::timeout(limit, child.wait_with_output()).await {
            ...
            Err(_)  => format!("Comando terminato: timeout di {}s superato", limit.as_secs()),
        };
